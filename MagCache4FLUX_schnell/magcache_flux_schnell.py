...
def magcache_forward(
        ...
        skip_forward = False
        if self.cnt in [2]: # 2,3 also can be removed, but the quality will decreate a little.
            cur_residual = self.previous_residual
            self.skip_steps += 1
            print(f"skip time {self.cnt}, skip steps {self.skip_steps}")
            skip_forward = True
...
# FluxTransformer2DModel.forward = magcache_forward
num_inference_steps = 4
seed = 42

pipeline = DiffusionPipeline.from_pretrained("/nfs/mzh/VideoGeneration/pretrained_weights/black-forest-labs/FLUX.1-schnell", torch_dtype=torch.bfloat16)
...
pipeline.transformer.__class__.mag_ratios = np.array([1.0]+[1.38281, 0.98438, 0.875])
pipeline.transformer.__class__.skip_steps = 0
# pipeline.transformer.__class__.forward = magcache_forward
pipeline.transformer.__class__.accumulated_sim = 1
...
for prompt in prompts:
    pipeline.transformer.__class__.cnt = 0
    # macs = profile_macs(pipeline, prompt, num_inference_steps=num_inference_steps,
